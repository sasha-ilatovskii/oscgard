...
-- 4. After 1.5s timeout, display in mod menu for user assignment
```
...
- `all(z)` — set all LEDs
- `refresh()` — throttled to 60Hz, sends hex state if buffer has dirty changes
...
function OscgardGrid:refresh()
    -- Skip if within 60Hz throttle window
    if (now - self.last_refresh_time) < self.refresh_interval then return end
...
- `refresh()` — throttled to 60Hz, sends ring state if changed
...
4. Provide module-level convenience functions (`grid.led`, `grid.refresh`, etc.) that operate on vport 1
5. Provide `cleanup()` to clear callbacks on script change
...
| Page | Purpose |
|------|---------|
| **main** | Lists connected devices. K3 to remove or add. |
| **type** | Select device type (grid / arc) |
| **port** | Select port (1-4, shows only free ports) |
| **discover** | Scan network, list available devices, K3 to assign |

Navigation: E2 to scroll, K2 to go back, K3 to select/action.
